...
impl std::fmt::Debug for EvictionLoop {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("EvictionLoop").finish_non_exhaustive()
	}
}
							...
							for candidate in candidates {
								...
								if let Err(error) = candidate.runtime.shutdown().await {
									tracing::warn!(
										key = ?candidate.key,
										?error,
										"failed graceful LSP shutdown; forcing stop"
									);
									...
									if let Err(force_error) = candidate.runtime.force_stop().await {
										tracing::error!(
											key = ?candidate.key,
											?force_error,
											"failed force-stop for idle LSP session"
										);
									}
								}
