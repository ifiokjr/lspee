...
pub enum ConfigAction {
	...
	Show(ShowCommand),
	/// Create an initial monokit.toml in the project root.
	Init(InitCommand),
...
pub struct InitCommand {
	...
	/// Overwrite existing monokit.toml.
	#[arg(long)]
	pub force: bool,
}
...
fn run_init(cmd: &InitCommand) -> anyhow::Result<()> {
	...
	let config_path = project_root.join("monokit.toml");

	if config_path.exists() && !cmd.force {
		anyhow::bail!(
			"monokit.toml already exists at {}. Use --force to overwrite.",
			config_path.display()
		);
	}
...
fn run_remove_lsp(cmd: &RemoveLspCommand) -> anyhow::Result<()> {
	...
	if !config_path.exists() {
		anyhow::bail!("no monokit.toml found at {}", config_path.display());
	}
	...
	match parts.as_slice() {
		[section, key] => {
			let table = doc[*section]
				...
				.as_table_mut()
				.ok_or_else(|| anyhow::anyhow!("'{section}' is not a table in monokit.toml"))?;
...
fn generate_init_template(project_root: &Path) -> String {
	...
	let mut output = String::from(
		"# monokit project configuration\n# Docs: https://github.com/ifiokjr/monokit\n\n",
	);
...
mod tests {
	...
	fn temp_dir(name: &str) -> PathBuf {
		let nanos = SystemTime::now()
			...
			.unwrap()
			.as_nanos();
		let dir = std::env::temp_dir().join(format!("monokit-config-test-{name}-{nanos}"));
	...
	fn add_lsp_to_empty_config() {
		...
		let config_path = dir.join("monokit.toml");
...
"#;
	...
	fn run_init_creates_file() {
		...
		assert!(result.is_ok());
		assert!(dir.join("monokit.toml").exists());
	...
	fn run_add_lsp_and_remove_lsp() {
		...
		fs::write(dir.join("monokit.toml"), "").unwrap();
		...
		let content = fs::read_to_string(dir.join("monokit.toml")).unwrap();
		assert!(content.contains("taplo"));
