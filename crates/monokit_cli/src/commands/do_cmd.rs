//! Ergonomic LSP method wrappers for the `monokit do` command.
//!
...
/// Controls the JSON output format for `monokit do` responses.
...
/// Arguments shared by all `monokit do` methods.
///
...
/// monokit do hover --lsp rust-analyzer --file src/main.rs --line 10 --col 5
/// ```
...
pub struct SharedArgs {
	...
	/// Use `monokit lsps --file <path>` to see which servers match a file.
	#[arg(long = "lsp")]
...
pub enum DoMethod {
	...
	/// Check `monokit capabilities --lsp <id>` to verify support.
...
/// Run the `monokit do` command synchronously.
pub fn run(cmd: DoCommand) -> Result<()> {
	let runtime = tokio::runtime::Runtime::new()?;
	runtime.block_on(run_async(cmd))
}

/// Async implementation of the `monokit do` command.
///
/// Follows the same attach → call → release lifecycle as `monokit call`,
...
async fn run_async(cmd: DoCommand) -> Result<()> {
	...
	// Resolve config.
	let resolved = monokit_config::resolve(shared.root.as_deref())?;
	...
	let attach = ControlEnvelope {
		...
		payload: serde_json::to_value(Attach {
			...
			client_meta: ClientMeta {
				client_name: "monokit_cli".to_string(),
...
mod tests {
	...
	fn unique_temp_dir(name: &str) -> PathBuf {
		let nanos = SystemTime::now()
			...
			.as_nanos();
		let dir = std::env::temp_dir().join(format!("monokit_do_test_{name}_{nanos}"));
	...
	fn short_temp_dir(name: &str) -> PathBuf {
		let nanos = SystemTime::now()
			...
			.expect("time")
			.as_nanos();
		let dir = PathBuf::from("/tmp").join(format!("monokit-do-{name}-{nanos}"));
	...
	fn write_cat_project_config(root: &Path) {
		let config = "workspace_mode = \"single\"\n\n[[lsp]]\nid = \"cat-lsp\"\ncommand = \
		              \"cat\"\nargs = []\n";
		fs::write(root.join("monokit.toml"), config).expect("write config");
	}
	...
	async fn shutdown_test_daemon(root: &Path) {
		...
		if let Ok(stream) = tokio::net::UnixStream::connect(&socket).await {
			...
			let env = ControlEnvelope {
				...
				payload: serde_json::to_value(monokit_daemon::Shutdown {}).unwrap(),
			};
