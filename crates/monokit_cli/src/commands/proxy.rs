...
use monokit_daemon::StreamErrorPayload;
use monokit_daemon::StreamFrame;
use monokit_daemon::StreamFrameType;
...
async fn run_async(cmd: ProxyCommand) -> anyhow::Result<()> {
	...
	let attach = ControlEnvelope {
		...
		payload: serde_json::to_value(Attach {
			...
			client_meta: ClientMeta {
				client_name: "monokit_proxy".to_string(),
	...
	loop {
		tokio::select! {
			incoming = monokit_lsp::read_lsp_frame(&mut stdin_reader) => {
				match incoming? {
					Some(message) => {
...
async fn write_lsp_message(stdout: &mut tokio::io::Stdout, message: &Value) -> anyhow::Result<()> {
	let frame = monokit_lsp::encode_lsp_frame(message)?;
...
fn stream_error_to_editor_warning(payload: &StreamErrorPayload) -> Value {
	...
	json!({
		...
		"params": {
			"type": 2,
			"message": format!("monokit warning: {} {}", payload.message, resume_hint),
		}
	})
}
