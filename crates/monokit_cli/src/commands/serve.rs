...
pub struct ServeCommand {
	...
	/// Log output format.
	#[arg(long, value_enum, env = "MONOKIT_LOG_FORMAT", default_value_t = LogFormat::Human)]
	pub log_format: LogFormat,

	/// Write logs to a file instead of stderr.
	#[arg(long, env = "MONOKIT_LOG_FILE")]
...
fn init_tracing(cmd: &ServeCommand) -> anyhow::Result<()> {
	let filter = EnvFilter::try_from_env("MONOKIT_LOG")
		.unwrap_or_else(|_| EnvFilter::new("monokit=info,warn"));
...
async fn run_async(cmd: ServeCommand) -> anyhow::Result<()> {
	let resolved = monokit_config::resolve(cmd.project_root.as_deref())?;
	let daemon = monokit_daemon::Daemon::new(resolved.project_root.clone(), resolved);
