#![cfg(unix)]

//! Integration tests that exercise CLI command functions against a live daemon.

use std::fs;
use std::path::Path;
use std::path::PathBuf;

use monokit_test_helpers::daemon::DaemonHandle;
use monokit_test_helpers::fixtures::setup_fixture;

// ---------------------------------------------------------------------------
// monokit lsp (exercises lsp.rs)
// ---------------------------------------------------------------------------

#[test]
fn lsp_command_runs_successfully() {
	let (_temp, root) = setup_fixture("rust-project");

	let result = monokit_cli::commands::lsp::run(&monokit_cli::commands::lsp::LspCommand {
		project_root: Some(root.clone()),
		output: monokit_cli::commands::lsp::LspOutput::Json,
	});
	assert!(result.is_ok());

	let result = monokit_cli::commands::lsp::run(&monokit_cli::commands::lsp::LspCommand {
		project_root: Some(root.clone()),
		output: monokit_cli::commands::lsp::LspOutput::Human,
	});
	assert!(result.is_ok());
}

// ---------------------------------------------------------------------------
// monokit lsps (exercises lsps.rs)
// ---------------------------------------------------------------------------

#[test]
fn lsps_command_with_file_json() {
	let (_temp, _root) = setup_fixture("rust-project");

	let result = monokit_cli::commands::lsps::run(monokit_cli::commands::lsps::LspsCommand {
		file: Some(PathBuf::from("src/main.rs")),
		output: monokit_cli::commands::lsps::LspsOutput::Json,
	});
	assert!(result.is_ok());
}

#[test]
fn lsps_command_with_file_human() {
	let result = monokit_cli::commands::lsps::run(monokit_cli::commands::lsps::LspsCommand {
		file: Some(PathBuf::from("src/main.rs")),
		output: monokit_cli::commands::lsps::LspsOutput::Human,
	});
	assert!(result.is_ok());
}

#[test]
fn lsps_command_without_file() {
	let result = monokit_cli::commands::lsps::run(monokit_cli::commands::lsps::LspsCommand {
		file: None,
		output: monokit_cli::commands::lsps::LspsOutput::Json,
	});
	assert!(result.is_ok());

	let result = monokit_cli::commands::lsps::run(monokit_cli::commands::lsps::LspsCommand {
		file: None,
		output: monokit_cli::commands::lsps::LspsOutput::Human,
	});
	assert!(result.is_ok());
}

#[test]
fn lsps_command_unknown_extension() {
	let result = monokit_cli::commands::lsps::run(monokit_cli::commands::lsps::LspsCommand {
		file: Some(PathBuf::from("file.zzzzzzz")),
		output: monokit_cli::commands::lsps::LspsOutput::Human,
	});
	assert!(result.is_ok());
}

// ---------------------------------------------------------------------------
// monokit doctor (exercises doctor.rs)
// ---------------------------------------------------------------------------

#[test]
fn doctor_command_json() {
	let (_temp, root) = setup_fixture("rust-project");

	let result = monokit_cli::commands::doctor::run(monokit_cli::commands::doctor::DoctorCommand {
		project_root: Some(root),
		output: monokit_cli::commands::doctor::DoctorOutput::Json,
	});
	assert!(result.is_ok());
}

#[test]
fn doctor_command_human() {
	let (_temp, root) = setup_fixture("rust-project");

	let result = monokit_cli::commands::doctor::run(monokit_cli::commands::doctor::DoctorCommand {
		project_root: Some(root),
		output: monokit_cli::commands::doctor::DoctorOutput::Human,
	});
	assert!(result.is_ok());
}

// ---------------------------------------------------------------------------
// monokit status (exercises status.rs + client.rs)
// ---------------------------------------------------------------------------

#[test]
fn status_command_json() {
	let (_temp, root) = setup_fixture("rust-project");
	let _daemon = DaemonHandle::start(&root);

	let result = monokit_cli::commands::status::run(monokit_cli::commands::status::StatusCommand {
		project_root: Some(root.clone()),
		no_start_daemon: false,
		output: monokit_cli::commands::status::StatusOutput::Json,
	});
	assert!(result.is_ok());

	let result = monokit_cli::commands::status::run(monokit_cli::commands::status::StatusCommand {
		project_root: Some(root),
		no_start_daemon: false,
		output: monokit_cli::commands::status::StatusOutput::Human,
	});
	assert!(result.is_ok());
}

// ---------------------------------------------------------------------------
// monokit call (exercises call.rs + client.rs)
// ---------------------------------------------------------------------------

#[test]
fn call_command_json_output() {
	let (_temp, root) = setup_fixture("rust-project");
	let _daemon = DaemonHandle::start(&root);

	let request = serde_json::json!({
		"jsonrpc": "2.0",
		"id": 1,
		"method": "textDocument/hover",
		"params": {}
	});

	let result = monokit_cli::commands::call::run(monokit_cli::commands::call::CallCommand {
		lsp: "rust-analyzer".to_string(),
		root: Some(root.clone()),
		request: serde_json::to_string(&request).unwrap(),
		no_start_daemon: false,
		client_kind: monokit_cli::commands::call::CallClientKind::Agent,
		output: monokit_cli::commands::call::CallOutput::Json,
	});
	assert!(result.is_ok());

	let result = monokit_cli::commands::call::run(monokit_cli::commands::call::CallCommand {
		lsp: "rust-analyzer".to_string(),
		root: Some(root),
		request: serde_json::to_string(&request).unwrap(),
		no_start_daemon: false,
		client_kind: monokit_cli::commands::call::CallClientKind::Human,
		output: monokit_cli::commands::call::CallOutput::Pretty,
	});
	assert!(result.is_ok());
}

#[test]
fn call_command_from_file() {
	let (_temp, root) = setup_fixture("rust-project");
	let _daemon = DaemonHandle::start(&root);

	let request = serde_json::json!({
		"jsonrpc": "2.0",
		"id": 2,
		"method": "workspace/symbol",
		"params": {"query": "test"}
	});
	let req_file = root.join("request.json");
	fs::write(&req_file, serde_json::to_string(&request).unwrap()).unwrap();

	let result = monokit_cli::commands::call::run(monokit_cli::commands::call::CallCommand {
		lsp: "rust-analyzer".to_string(),
		root: Some(root),
		request: format!("@{}", req_file.display()),
		no_start_daemon: false,
		client_kind: monokit_cli::commands::call::CallClientKind::Ci,
		output: monokit_cli::commands::call::CallOutput::Json,
	});
	assert!(result.is_ok());
}

// ---------------------------------------------------------------------------
// monokit stop (exercises stop.rs + client.rs)
// ---------------------------------------------------------------------------

#[test]
fn stop_command_stops_daemon() {
	let (_temp, root) = setup_fixture("rust-project");
	let daemon = DaemonHandle::start(&root);

	let result = monokit_cli::commands::stop::run(monokit_cli::commands::stop::StopCommand {
		project_root: Some(root),
	});
	assert!(result.is_ok());

	// Daemon was already stopped via the stop command; just drop the handle.
	drop(daemon);
}

#[test]
fn stop_command_when_daemon_not_running() {
	let (_temp, root) = setup_fixture("rust-project");

	let result = monokit_cli::commands::stop::run(monokit_cli::commands::stop::StopCommand {
		project_root: Some(root),
	});
	assert!(result.is_ok());
}

// ---------------------------------------------------------------------------
// monokit config (exercises config.rs)
// ---------------------------------------------------------------------------

#[test]
fn config_show_json() {
	let (_temp, root) = setup_fixture("rust-project");

	let result =
		monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
			action: monokit_cli::commands::config::ConfigAction::Show(
				monokit_cli::commands::config::ShowCommand {
					root: Some(root.clone()),
					output: monokit_cli::commands::config::ConfigOutput::Json,
				},
			),
		});
	assert!(result.is_ok());

	let result =
		monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
			action: monokit_cli::commands::config::ConfigAction::Show(
				monokit_cli::commands::config::ShowCommand {
					root: Some(root),
					output: monokit_cli::commands::config::ConfigOutput::Human,
				},
			),
		});
	assert!(result.is_ok());
}

// ---------------------------------------------------------------------------
// Protocol edge cases
// ---------------------------------------------------------------------------

async fn shutdown_daemon(root: &Path) {
	use monokit_daemon::ControlEnvelope;
	use monokit_daemon::PROTOCOL_VERSION;
	use monokit_daemon::Shutdown;
	use monokit_daemon::TYPE_SHUTDOWN;
	use tokio::io::AsyncBufReadExt;
	use tokio::io::AsyncWriteExt;
	use tokio::io::BufReader;

	let socket = root.join(".monokit").join("daemon.sock");
	if let Ok(stream) = tokio::net::UnixStream::connect(&socket).await {
		let (reader, mut writer) = stream.into_split();
		let mut lines = BufReader::new(reader).lines();
		let envelope = ControlEnvelope {
			v: PROTOCOL_VERSION,
			id: Some("shutdown".to_string()),
			message_type: TYPE_SHUTDOWN.to_string(),
			payload: serde_json::to_value(Shutdown::default()).unwrap(),
		};
		let mut bytes = serde_json::to_vec(&envelope).unwrap();
		bytes.push(b'\n');

		let _ = writer.write_all(&bytes).await;
		let _ = writer.flush().await;
		let _ = lines.next_line().await;
	}
}

fn spawn_daemon(root: &Path) -> tokio::task::JoinHandle<anyhow::Result<()>> {
	let resolved = monokit_config::resolve(Some(root)).expect("config should resolve");
	let daemon = monokit_daemon::Daemon::new(root.to_path_buf(), resolved);

	tokio::spawn(async move { daemon.run().await })
}

async fn wait_for_socket(root: &Path) {
	let socket = root.join(".monokit").join("daemon.sock");

	for _ in 0..100 {
		if tokio::net::UnixStream::connect(&socket).await.is_ok() {
			return;
		}
		tokio::time::sleep(std::time::Duration::from_millis(25)).await;
	}

	panic!("daemon socket did not become available");
}

#[tokio::test]
async fn daemon_rejects_invalid_json() {
	let (_temp, root) = setup_fixture("rust-project");

	let daemon_task = spawn_daemon(&root);
	wait_for_socket(&root).await;

	use monokit_daemon::TYPE_ERROR;
	use tokio::io::AsyncBufReadExt;
	use tokio::io::AsyncWriteExt;
	use tokio::io::BufReader;

	let socket = root.join(".monokit").join("daemon.sock");
	let stream = tokio::net::UnixStream::connect(&socket).await.unwrap();
	let (reader, mut writer) = stream.into_split();
	let mut lines = BufReader::new(reader).lines();

	writer.write_all(b"not json\n").await.unwrap();
	writer.flush().await.unwrap();

	let line = lines.next_line().await.unwrap().unwrap();
	let response: monokit_daemon::ControlEnvelope<serde_json::Value> =
		serde_json::from_str(&line).unwrap();

	assert_eq!(response.message_type, TYPE_ERROR);
	assert_eq!(response.payload["code"], "E_BAD_MESSAGE");

	shutdown_daemon(&root).await;

	let _ = daemon_task.await;
}

#[tokio::test]
async fn daemon_rejects_wrong_protocol_version() {
	let (_temp, root) = setup_fixture("rust-project");

	let daemon_task = spawn_daemon(&root);
	wait_for_socket(&root).await;

	use monokit_daemon::TYPE_ERROR;
	use tokio::io::AsyncBufReadExt;
	use tokio::io::AsyncWriteExt;
	use tokio::io::BufReader;

	let socket = root.join(".monokit").join("daemon.sock");
	let stream = tokio::net::UnixStream::connect(&socket).await.unwrap();
	let (reader, mut writer) = stream.into_split();
	let mut lines = BufReader::new(reader).lines();

	let bad = serde_json::json!({"v": 999, "id": "bad", "type": "Stats", "payload": {}});
	let mut bytes = serde_json::to_vec(&bad).unwrap();
	bytes.push(b'\n');

	writer.write_all(&bytes).await.unwrap();
	writer.flush().await.unwrap();

	let line = lines.next_line().await.unwrap().unwrap();
	let response: monokit_daemon::ControlEnvelope<serde_json::Value> =
		serde_json::from_str(&line).unwrap();

	assert_eq!(response.message_type, TYPE_ERROR);
	assert_eq!(response.payload["code"], "E_UNSUPPORTED_VERSION");

	shutdown_daemon(&root).await;

	let _ = daemon_task.await;
}

#[tokio::test]
async fn daemon_rejects_unknown_message_type() {
	let (_temp, root) = setup_fixture("rust-project");

	let daemon_task = spawn_daemon(&root);
	wait_for_socket(&root).await;

	use monokit_daemon::PROTOCOL_VERSION;
	use monokit_daemon::TYPE_ERROR;
	use tokio::io::AsyncBufReadExt;
	use tokio::io::AsyncWriteExt;
	use tokio::io::BufReader;

	let socket = root.join(".monokit").join("daemon.sock");
	let stream = tokio::net::UnixStream::connect(&socket).await.unwrap();
	let (reader, mut writer) = stream.into_split();
	let mut lines = BufReader::new(reader).lines();

	let bad = serde_json::json!({"v": PROTOCOL_VERSION, "id": "x", "type": "Bogus", "payload": {}});
	let mut bytes = serde_json::to_vec(&bad).unwrap();
	bytes.push(b'\n');

	writer.write_all(&bytes).await.unwrap();
	writer.flush().await.unwrap();

	let line = lines.next_line().await.unwrap().unwrap();
	let response: monokit_daemon::ControlEnvelope<serde_json::Value> =
		serde_json::from_str(&line).unwrap();

	assert_eq!(response.message_type, TYPE_ERROR);
	assert_eq!(response.payload["code"], "E_UNKNOWN_TYPE");

	shutdown_daemon(&root).await;

	let _ = daemon_task.await;
}

#[tokio::test]
async fn daemon_rejects_invalid_session_key() {
	let (_temp, root) = setup_fixture("rust-project");

	let daemon_task = spawn_daemon(&root);
	wait_for_socket(&root).await;

	use monokit_daemon::Attach;
	use monokit_daemon::AttachCapabilities;
	use monokit_daemon::ClientKind;
	use monokit_daemon::ClientMeta;
	use monokit_daemon::ControlEnvelope;
	use monokit_daemon::PROTOCOL_VERSION;
	use monokit_daemon::SessionKeyWire;
	use monokit_daemon::StreamMode;
	use monokit_daemon::TYPE_ATTACH;
	use monokit_daemon::TYPE_ERROR;
	use tokio::io::AsyncBufReadExt;
	use tokio::io::AsyncWriteExt;
	use tokio::io::BufReader;

	let socket = root.join(".monokit").join("daemon.sock");
	let stream = tokio::net::UnixStream::connect(&socket).await.unwrap();
	let (reader, mut writer) = stream.into_split();
	let mut lines = BufReader::new(reader).lines();

	let attach = ControlEnvelope {
		v: PROTOCOL_VERSION,
		id: Some("bad-attach".to_string()),
		message_type: TYPE_ATTACH.to_string(),
		payload: serde_json::to_value(Attach {
			session_key: SessionKeyWire {
				project_root: root.display().to_string(),
				config_hash: "hash".to_string(),
				lsp_id: String::new(),
			},
			client_meta: ClientMeta {
				client_name: "test".to_string(),
				client_version: "0.1.0".to_string(),
				client_kind: Some(ClientKind::Agent),
				pid: None,
				cwd: None,
			},
			capabilities: Some(AttachCapabilities {
				stream_mode: vec![StreamMode::MuxControl],
			}),
		})
		.unwrap(),
	};
	let mut bytes = serde_json::to_vec(&attach).unwrap();
	bytes.push(b'\n');

	writer.write_all(&bytes).await.unwrap();
	writer.flush().await.unwrap();

	let line = lines.next_line().await.unwrap().unwrap();
	let response: ControlEnvelope<serde_json::Value> = serde_json::from_str(&line).unwrap();

	assert_eq!(response.message_type, TYPE_ERROR);
	assert_eq!(response.payload["code"], "E_INVALID_SESSION_KEY");

	shutdown_daemon(&root).await;

	let _ = daemon_task.await;
}

#[tokio::test]
async fn release_nonexistent_lease_returns_error() {
	let (_temp, root) = setup_fixture("rust-project");

	let daemon_task = spawn_daemon(&root);
	wait_for_socket(&root).await;

	use monokit_daemon::ControlEnvelope;
	use monokit_daemon::PROTOCOL_VERSION;
	use monokit_daemon::Release;
	use monokit_daemon::TYPE_ERROR;
	use monokit_daemon::TYPE_RELEASE;
	use tokio::io::AsyncBufReadExt;
	use tokio::io::AsyncWriteExt;
	use tokio::io::BufReader;

	let socket = root.join(".monokit").join("daemon.sock");
	let stream = tokio::net::UnixStream::connect(&socket).await.unwrap();
	let (reader, mut writer) = stream.into_split();
	let mut lines = BufReader::new(reader).lines();

	let release = ControlEnvelope {
		v: PROTOCOL_VERSION,
		id: Some("release-bad".to_string()),
		message_type: TYPE_RELEASE.to_string(),
		payload: serde_json::to_value(Release {
			lease_id: "nonexistent_lease".to_string(),
			reason: None,
		})
		.unwrap(),
	};
	let mut bytes = serde_json::to_vec(&release).unwrap();
	bytes.push(b'\n');

	writer.write_all(&bytes).await.unwrap();
	writer.flush().await.unwrap();

	let line = lines.next_line().await.unwrap().unwrap();
	let response: ControlEnvelope<serde_json::Value> = serde_json::from_str(&line).unwrap();

	assert_eq!(response.message_type, TYPE_ERROR);
	assert_eq!(response.payload["code"], "E_LEASE_NOT_FOUND");

	shutdown_daemon(&root).await;

	let _ = daemon_task.await;
}

// ---------------------------------------------------------------------------
// Client helpers
// ---------------------------------------------------------------------------

#[test]
fn client_new_request_id_has_prefix() {
	let id = monokit_cli::commands::client::new_request_id("test");
	assert!(id.starts_with("test-"));
}

#[test]
fn client_daemon_socket_path() {
	let path = monokit_cli::commands::client::daemon_socket_path(Path::new("/my/project"));
	assert_eq!(path, PathBuf::from("/my/project/.monokit/daemon.sock"));
}

#[test]
fn client_render_error_payload() {
	let payload = serde_json::json!({"code": "E_TEST", "message": "test error"});
	let rendered = monokit_cli::commands::client::render_error_payload(&payload);

	assert!(rendered.contains("E_TEST"));
	assert!(rendered.contains("test error"));
}

#[test]
fn client_render_error_payload_missing_fields() {
	let payload = serde_json::json!({});
	let rendered = monokit_cli::commands::client::render_error_payload(&payload);

	assert!(rendered.contains("E_UNKNOWN"));
}

#[test]
fn client_ensure_not_error_passes_for_non_error() {
	let envelope = monokit_daemon::ControlEnvelope {
		v: monokit_daemon::PROTOCOL_VERSION,
		id: Some("test".to_string()),
		message_type: "AttachOk".to_string(),
		payload: serde_json::json!({}),
	};

	assert!(monokit_cli::commands::client::ensure_not_error(&envelope).is_ok());
}

#[test]
fn client_ensure_not_error_fails_for_error() {
	let envelope = monokit_daemon::ControlEnvelope {
		v: monokit_daemon::PROTOCOL_VERSION,
		id: Some("test".to_string()),
		message_type: "Error".to_string(),
		payload: serde_json::json!({"code": "E_TEST", "message": "fail"}),
	};

	assert!(monokit_cli::commands::client::ensure_not_error(&envelope).is_err());
}

// ---------------------------------------------------------------------------
// monokit capabilities (exercises capabilities.rs)
// ---------------------------------------------------------------------------

#[test]
fn capabilities_command_runs_successfully() {
	let (_temp, root) = setup_fixture("rust-project");
	let _daemon = DaemonHandle::start(&root);

	let result = monokit_cli::commands::capabilities::run(
		monokit_cli::commands::capabilities::CapabilitiesCommand {
			lsp: "rust-analyzer".to_string(),
			root: Some(root.clone()),
			no_start_daemon: false,
			output: monokit_cli::commands::capabilities::CapabilitiesOutput::Json,
		},
	);
	assert!(result.is_ok());

	let result = monokit_cli::commands::capabilities::run(
		monokit_cli::commands::capabilities::CapabilitiesCommand {
			lsp: "rust-analyzer".to_string(),
			root: Some(root),
			no_start_daemon: false,
			output: monokit_cli::commands::capabilities::CapabilitiesOutput::Human,
		},
	);
	assert!(result.is_ok());
}

// ---------------------------------------------------------------------------
// monokit restart (exercises restart.rs)
// ---------------------------------------------------------------------------

#[test]
fn restart_command_restarts_daemon() {
	let (_temp, root) = setup_fixture("rust-project");
	let daemon = DaemonHandle::start(&root);

	// Restart creates its own runtime, so call from sync context.
	// The restart will shut down the existing daemon and start a new one
	// via auto-start. But since our test daemon won't auto-start from the CLI,
	// the restart command should at least complete the shutdown part.
	// We test the shutdown + reconnect flow.
	let result =
		monokit_cli::commands::restart::run(monokit_cli::commands::restart::RestartCommand {
			project_root: Some(root),
		});
	// Restart may fail on reconnect since our test daemon is gone after shutdown,
	// but the shutdown path is exercised regardless.
	let _ = result;

	drop(daemon);
}

// ---------------------------------------------------------------------------
// monokit serve tracing init (exercises serve.rs)
// ---------------------------------------------------------------------------

#[test]
fn serve_log_format_values() {
	// Exercise the LogFormat enum variants exist and can be constructed
	let _human = monokit_cli::commands::serve::LogFormat::Human;
	let _json = monokit_cli::commands::serve::LogFormat::Json;
}

// ---------------------------------------------------------------------------
// Config edge cases
// ---------------------------------------------------------------------------

#[test]
fn config_init_then_add_remove_lsp() {
	let root = tempfile::tempdir().expect("should create temp dir");
	let root = fs::canonicalize(root.path()).expect("should canonicalize");

	// Init
	monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
		action: monokit_cli::commands::config::ConfigAction::Init(
			monokit_cli::commands::config::InitCommand {
				root: Some(root.clone()),
				force: false,
			},
		),
	})
	.unwrap();
	assert!(root.join("monokit.toml").exists());

	// Add LSP
	monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
		action: monokit_cli::commands::config::ConfigAction::AddLsp(
			monokit_cli::commands::config::AddLspCommand {
				id: "taplo".to_string(),
				command: "taplo".to_string(),
				args: Some(vec!["lsp".to_string(), "stdio".to_string()]),
				root: Some(root.clone()),
			},
		),
	})
	.unwrap();

	let content = fs::read_to_string(root.join("monokit.toml")).unwrap();
	assert!(content.contains("taplo"));

	// Remove LSP
	monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
		action: monokit_cli::commands::config::ConfigAction::RemoveLsp(
			monokit_cli::commands::config::RemoveLspCommand {
				id: "taplo".to_string(),
				root: Some(root.clone()),
			},
		),
	})
	.unwrap();

	let content = fs::read_to_string(root.join("monokit.toml")).unwrap();
	assert!(!content.contains("taplo"));

	// Set value
	monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
		action: monokit_cli::commands::config::ConfigAction::Set(
			monokit_cli::commands::config::SetCommand {
				key: "session.idle_ttl_secs".to_string(),
				value: "900".to_string(),
				root: Some(root.clone()),
			},
		),
	})
	.unwrap();

	let content = fs::read_to_string(root.join("monokit.toml")).unwrap();
	assert!(content.contains("900"));
}

#[test]
fn config_remove_lsp_nonexistent_fails() {
	let temp = tempfile::tempdir().expect("should create temp dir");
	let root = fs::canonicalize(temp.path()).expect("should canonicalize");
	fs::write(
		root.join("monokit.toml"),
		"[[lsp]]\nid = \"rust-analyzer\"\ncommand = \"ra\"\n",
	)
	.unwrap();

	let result =
		monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
			action: monokit_cli::commands::config::ConfigAction::RemoveLsp(
				monokit_cli::commands::config::RemoveLspCommand {
					id: "nonexistent".to_string(),
					root: Some(root),
				},
			),
		});
	assert!(result.is_err());
}

#[test]
fn config_remove_lsp_no_config_fails() {
	let temp = tempfile::tempdir().expect("should create temp dir");
	let root = fs::canonicalize(temp.path()).expect("should canonicalize");

	let result =
		monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
			action: monokit_cli::commands::config::ConfigAction::RemoveLsp(
				monokit_cli::commands::config::RemoveLspCommand {
					id: "anything".to_string(),
					root: Some(root),
				},
			),
		});
	assert!(result.is_err());
}

#[test]
fn config_set_top_level_key() {
	let temp = tempfile::tempdir().expect("should create temp dir");
	let root = fs::canonicalize(temp.path()).expect("should canonicalize");
	fs::write(root.join("monokit.toml"), "").unwrap();

	monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
		action: monokit_cli::commands::config::ConfigAction::Set(
			monokit_cli::commands::config::SetCommand {
				key: "workspace_mode".to_string(),
				value: "multi".to_string(),
				root: Some(root.clone()),
			},
		),
	})
	.unwrap();

	let content = fs::read_to_string(root.join("monokit.toml")).unwrap();
	assert!(content.contains("multi"));
}

#[test]
fn config_set_deep_key_fails() {
	let temp = tempfile::tempdir().expect("should create temp dir");
	let root = fs::canonicalize(temp.path()).expect("should canonicalize");
	fs::write(root.join("monokit.toml"), "").unwrap();

	let result =
		monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
			action: monokit_cli::commands::config::ConfigAction::Set(
				monokit_cli::commands::config::SetCommand {
					key: "a.b.c".to_string(),
					value: "x".to_string(),
					root: Some(root),
				},
			),
		});
	assert!(result.is_err());
}

#[test]
fn config_init_force_overwrites() {
	let temp = tempfile::tempdir().expect("should create temp dir");
	let root = fs::canonicalize(temp.path()).expect("should canonicalize");
	fs::write(root.join("monokit.toml"), "old content").unwrap();

	monokit_cli::commands::config::run(&monokit_cli::commands::config::ConfigCommand {
		action: monokit_cli::commands::config::ConfigAction::Init(
			monokit_cli::commands::config::InitCommand {
				root: Some(root.clone()),
				force: true,
			},
		),
	})
	.unwrap();

	let content = fs::read_to_string(root.join("monokit.toml")).unwrap();
	assert!(!content.contains("old content"));
}

// ---------------------------------------------------------------------------
// Additional registry/daemon coverage via protocol
// ---------------------------------------------------------------------------

#[tokio::test]
async fn daemon_attach_bad_payload() {
	let (_temp, root) = setup_fixture("rust-project");

	let daemon_task = spawn_daemon(&root);
	wait_for_socket(&root).await;

	use monokit_daemon::ControlEnvelope;
	use monokit_daemon::PROTOCOL_VERSION;
	use monokit_daemon::TYPE_ATTACH;
	use monokit_daemon::TYPE_ERROR;
	use tokio::io::AsyncBufReadExt;
	use tokio::io::AsyncWriteExt;
	use tokio::io::BufReader;

	let socket = root.join(".monokit").join("daemon.sock");
	let stream = tokio::net::UnixStream::connect(&socket).await.unwrap();
	let (reader, mut writer) = stream.into_split();
	let mut lines = BufReader::new(reader).lines();

	// Send Attach with invalid payload (not an Attach struct)
	let envelope = ControlEnvelope {
		v: PROTOCOL_VERSION,
		id: Some("bad-payload".to_string()),
		message_type: TYPE_ATTACH.to_string(),
		payload: serde_json::json!("not an attach struct"),
	};
	let mut bytes = serde_json::to_vec(&envelope).unwrap();
	bytes.push(b'\n');

	writer.write_all(&bytes).await.unwrap();
	writer.flush().await.unwrap();

	let line = lines.next_line().await.unwrap().unwrap();
	let resp: ControlEnvelope<serde_json::Value> = serde_json::from_str(&line).unwrap();

	assert_eq!(resp.message_type, TYPE_ERROR);
	assert_eq!(resp.payload["code"], "E_BAD_MESSAGE");

	shutdown_daemon(&root).await;

	let _ = daemon_task.await;
}

#[tokio::test]
async fn daemon_call_bad_payload() {
	let (_temp, root) = setup_fixture("rust-project");

	let daemon_task = spawn_daemon(&root);
	wait_for_socket(&root).await;

	use monokit_daemon::ControlEnvelope;
	use monokit_daemon::PROTOCOL_VERSION;
	use monokit_daemon::TYPE_CALL;
	use monokit_daemon::TYPE_ERROR;
	use tokio::io::AsyncBufReadExt;
	use tokio::io::AsyncWriteExt;
	use tokio::io::BufReader;

	let socket = root.join(".monokit").join("daemon.sock");
	let stream = tokio::net::UnixStream::connect(&socket).await.unwrap();
	let (reader, mut writer) = stream.into_split();
	let mut lines = BufReader::new(reader).lines();

	let envelope = ControlEnvelope {
		v: PROTOCOL_VERSION,
		id: Some("bad-call".to_string()),
		message_type: TYPE_CALL.to_string(),
		payload: serde_json::json!(42),
	};
	let mut bytes = serde_json::to_vec(&envelope).unwrap();
	bytes.push(b'\n');

	writer.write_all(&bytes).await.unwrap();
	writer.flush().await.unwrap();

	let line = lines.next_line().await.unwrap().unwrap();
	let resp: ControlEnvelope<serde_json::Value> = serde_json::from_str(&line).unwrap();

	assert_eq!(resp.message_type, TYPE_ERROR);

	shutdown_daemon(&root).await;

	let _ = daemon_task.await;
}

#[tokio::test]
async fn daemon_release_bad_payload() {
	let (_temp, root) = setup_fixture("rust-project");

	let daemon_task = spawn_daemon(&root);
	wait_for_socket(&root).await;

	use monokit_daemon::ControlEnvelope;
	use monokit_daemon::PROTOCOL_VERSION;
	use monokit_daemon::TYPE_ERROR;
	use monokit_daemon::TYPE_RELEASE;
	use tokio::io::AsyncBufReadExt;
	use tokio::io::AsyncWriteExt;
	use tokio::io::BufReader;

	let socket = root.join(".monokit").join("daemon.sock");
	let stream = tokio::net::UnixStream::connect(&socket).await.unwrap();
	let (reader, mut writer) = stream.into_split();
	let mut lines = BufReader::new(reader).lines();

	let envelope = ControlEnvelope {
		v: PROTOCOL_VERSION,
		id: Some("bad-rel".to_string()),
		message_type: TYPE_RELEASE.to_string(),
		payload: serde_json::json!("nope"),
	};
	let mut bytes = serde_json::to_vec(&envelope).unwrap();
	bytes.push(b'\n');

	writer.write_all(&bytes).await.unwrap();
	writer.flush().await.unwrap();

	let line = lines.next_line().await.unwrap().unwrap();
	let resp: ControlEnvelope<serde_json::Value> = serde_json::from_str(&line).unwrap();

	assert_eq!(resp.message_type, TYPE_ERROR);

	shutdown_daemon(&root).await;

	let _ = daemon_task.await;
}

#[tokio::test]
async fn daemon_shutdown_bad_payload() {
	let (_temp, root) = setup_fixture("rust-project");

	let daemon_task = spawn_daemon(&root);
	wait_for_socket(&root).await;

	use monokit_daemon::ControlEnvelope;
	use monokit_daemon::PROTOCOL_VERSION;
	use monokit_daemon::TYPE_ERROR;
	use monokit_daemon::TYPE_SHUTDOWN;
	use tokio::io::AsyncBufReadExt;
	use tokio::io::AsyncWriteExt;
	use tokio::io::BufReader;

	let socket = root.join(".monokit").join("daemon.sock");
	let stream = tokio::net::UnixStream::connect(&socket).await.unwrap();
	let (reader, mut writer) = stream.into_split();
	let mut lines = BufReader::new(reader).lines();

	let envelope = ControlEnvelope {
		v: PROTOCOL_VERSION,
		id: Some("bad-shut".to_string()),
		message_type: TYPE_SHUTDOWN.to_string(),
		payload: serde_json::json!("not a shutdown"),
	};
	let mut bytes = serde_json::to_vec(&envelope).unwrap();
	bytes.push(b'\n');

	writer.write_all(&bytes).await.unwrap();
	writer.flush().await.unwrap();

	let line = lines.next_line().await.unwrap().unwrap();
	let resp: ControlEnvelope<serde_json::Value> = serde_json::from_str(&line).unwrap();

	assert_eq!(resp.message_type, TYPE_ERROR);

	shutdown_daemon(&root).await;

	let _ = daemon_task.await;
}

// ---------------------------------------------------------------------------
// mod.rs dispatch (exercises run() function)
// ---------------------------------------------------------------------------

#[test]
fn dispatch_lsp_command_via_run() {
	let (_temp, root) = setup_fixture("rust-project");
	let result = monokit_cli::commands::run(monokit_cli::commands::Command::Lsp(
		monokit_cli::commands::lsp::LspCommand {
			project_root: Some(root),
			output: monokit_cli::commands::lsp::LspOutput::Json,
		},
	));
	assert!(result.is_ok());
}

#[test]
fn dispatch_lsps_command_via_run() {
	let result = monokit_cli::commands::run(monokit_cli::commands::Command::Lsps(
		monokit_cli::commands::lsps::LspsCommand {
			file: Some(PathBuf::from("main.rs")),
			output: monokit_cli::commands::lsps::LspsOutput::Json,
		},
	));
	assert!(result.is_ok());
}

#[test]
fn dispatch_doctor_via_run() {
	let (_temp, root) = setup_fixture("rust-project");
	let result = monokit_cli::commands::run(monokit_cli::commands::Command::Doctor(
		monokit_cli::commands::doctor::DoctorCommand {
			project_root: Some(root),
			output: monokit_cli::commands::doctor::DoctorOutput::Json,
		},
	));
	assert!(result.is_ok());
}

#[test]
fn dispatch_config_via_run() {
	let (_temp, root) = setup_fixture("rust-project");
	let result = monokit_cli::commands::run(monokit_cli::commands::Command::Config(
		monokit_cli::commands::config::ConfigCommand {
			action: monokit_cli::commands::config::ConfigAction::Show(
				monokit_cli::commands::config::ShowCommand {
					root: Some(root),
					output: monokit_cli::commands::config::ConfigOutput::Json,
				},
			),
		},
	));
	assert!(result.is_ok());
}

#[test]
fn dispatch_stop_via_run() {
	let (_temp, root) = setup_fixture("rust-project");
	let result = monokit_cli::commands::run(monokit_cli::commands::Command::Stop(
		monokit_cli::commands::stop::StopCommand {
			project_root: Some(root),
		},
	));
	assert!(result.is_ok());
}

#[test]
fn dispatch_status_call_caps_via_run() {
	let (_temp, root) = setup_fixture("rust-project");
	let _daemon = DaemonHandle::start(&root);

	let _ = monokit_cli::commands::run(monokit_cli::commands::Command::Status(
		monokit_cli::commands::status::StatusCommand {
			project_root: Some(root.clone()),
			no_start_daemon: false,
			output: monokit_cli::commands::status::StatusOutput::Json,
		},
	));

	let request = serde_json::json!({"jsonrpc":"2.0","id":1,"method":"test","params":{}});
	let _ = monokit_cli::commands::run(monokit_cli::commands::Command::Call(
		monokit_cli::commands::call::CallCommand {
			lsp: "rust-analyzer".to_string(),
			root: Some(root.clone()),
			request: serde_json::to_string(&request).unwrap(),
			no_start_daemon: false,
			client_kind: monokit_cli::commands::call::CallClientKind::Agent,
			output: monokit_cli::commands::call::CallOutput::Json,
		},
	));

	let _ = monokit_cli::commands::run(monokit_cli::commands::Command::Capabilities(
		monokit_cli::commands::capabilities::CapabilitiesCommand {
			lsp: "rust-analyzer".to_string(),
			root: Some(root),
			no_start_daemon: false,
			output: monokit_cli::commands::capabilities::CapabilitiesOutput::Json,
		},
	));
}
