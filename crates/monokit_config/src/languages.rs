...
///
/// ```
/// use monokit_config::languages::lsp_for_id;
///
...
/// ```
/// use std::path::Path;
/// use monokit_config::languages::lsps_for_file;
///
...
fn load_registry(
	...
	let mut registry: LanguageRegistryFile =
		toml::from_str(DEFAULT_LANGUAGES_TOML).map_err(|source| {
			ConfigError::Parse {
				path: Path::new("crates/monokit_config/defaults/languages.toml").to_path_buf(),
				source,
			}
		})?;
...
mod tests {
	...
	use super::DEFAULT_LANGUAGES_TOML;
	use super::LanguageRegistryFile;
	use super::lsp_for_id;
	use super::lsps_for_file;
	...
	fn unique_temp_file(name: &str) -> PathBuf {
		...
		std::env::temp_dir().join(format!("monokit-{name}-{nanos}.toml"))
	}
