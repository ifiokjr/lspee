...
#[cfg(not(unix))]
compile_error!("monokit_daemon currently supports unix-like platforms only (linux/macOS)");
...
pub struct Daemon {
	root: PathBuf,
	config: monokit_config::ResolvedConfig,
...
impl Daemon {
	#[must_use]
	pub fn new(root: PathBuf, config: monokit_config::ResolvedConfig) -> Self {
	...
	fn control_socket_path(&self) -> PathBuf {
		self.root.join(".monokit").join("daemon.sock")
	}
...
async fn dispatch_attach(
	...
	let response = match registry
		.acquire_or_spawn(key.clone(), client_kind, move |spawn_key| {
			...
			async move {
				let resolved =
					monokit_config::resolve(Some(&spawn_root)).map_err(anyhow::Error::from)?;

				let lsp_config = resolve_lsp_config(&resolved, &spawn_root, &spawn_lsp_id)?;

				let transport = Arc::new(monokit_lsp::LspTransport::new(spawn_root.clone()));
	...
	{
		Ok(lease) => {
			let Some(handle) = registry.session_handle(&key).await else {
				return DispatchOutcome {
					response: error_envelope(
						id,
						ERROR_INTERNAL,
			...
			let stream = match requested_stream_mode {
				StreamMode::Dedicated => {
					match stream::spawn_dedicated_stream_endpoint(
						&key.root,
						lease.lease_id(),
						handle.clone(),
						registry.clone(),
...
fn resolve_lsp_config(
...
) -> Result<monokit_config::LspConfig> {
	let user_config = std::env::var_os("HOME")
		.map(PathBuf::from)
		.map(|home| home.join(".config/monokit/config.toml"));
	let project_config = project_root.join("monokit.toml");

	let catalog = monokit_config::languages::lsp_for_id(
		requested_lsp_id,
	...
	if let Some(catalog) = catalog {
		return Ok(monokit_config::LspConfig {
	...
	Err(anyhow!(
		"no command found for lsp id '{requested_lsp_id}'. add a [[lsp]] entry or install the \
		 server"
	))
}
...
async fn bootstrap_lsp_session(
	runtime: &monokit_lsp::LspRuntime,
	project_root: &Path,
	lsp: &monokit_config::LspConfig,
) -> Result<Value> {
	let mut params = json!({
		"processId": null,
		"capabilities": {}
