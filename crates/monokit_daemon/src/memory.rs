...
use monokit_protocol::ClientKind;
use monokit_protocol::ERROR_SESSION_EVICTED_MEMORY;
use monokit_protocol::StreamErrorPayload;
use serde_json::json;
use tokio::sync::oneshot;
use tokio::task::JoinHandle;
use tokio::time;

use crate::registry::MemorySessionSnapshot;
use crate::registry::SessionRegistry;

#[derive(Debug, Clone, Copy)]
pub(crate) struct MemoryBudgetSettings {
	pub(crate) max_session_bytes: Option<u64>,
...
impl MemoryMonitor {
	pub(crate) fn start(registry: SessionRegistry, settings: MemoryBudgetSettings) -> Self {
...
pub(crate) struct MemorySample {
	pub(crate) key: crate::SessionKey,
	pub(crate) root: PathBuf,
	pub(crate) rss_bytes: u64,
	pub(crate) ref_count: usize,
	pub(crate) idle_for: Duration,
	pub(crate) client_kinds: Vec<ClientKind>,
	pub(crate) handle: crate::SessionHandle,
}

pub(crate) async fn total_memory_bytes(registry: &SessionRegistry) -> u64 {
...
async fn evict_session(
	...
	let notice = StreamErrorPayload {
		...
		message: format!(
			"Session evicted due to memory budget pressure (rss={} bytes). Re-attach or retry the \
			 request to resume.",
			sample.rss_bytes
		),
		retryable: true,
...
mod tests {
	...
	use monokit_config::LspConfig;
	use monokit_protocol::ClientKind;

	use super::MemoryBudgetSettings;
	use super::MemorySample;
	use super::select_evictions;
	...
	async fn sample(
		...
		let transport = std::sync::Arc::new(monokit_lsp::LspTransport::new(key.root.clone()));
