...
fn unique_temp_dir(name: &str) -> PathBuf {
	let nanos = SystemTime::now()
		...
		.as_nanos();
	let dir = PathBuf::from("/tmp").join(format!("monokit-daemon-test-{name}-{nanos}"));
...
"#
	...
	fs::write(root.join("monokit.toml"), config).expect("project config should be written");
}
...
async fn attach_call_release_and_stats_flow() {
	...
	let socket_path = root.join(".monokit").join("daemon.sock");
...
async fn dedicated_stream_forwards_bidirectional_lsp_frames() {
	...
	let request_frame = monokit_daemon::StreamFrame {
		v: monokit_daemon::PROTOCOL_VERSION,
		frame_type: monokit_daemon::StreamFrameType::LspIn,
		lease_id: lease_id.clone(),
		seq: 1,
		payload: json!({
			"jsonrpc": "2.0",
	...
	let response: monokit_daemon::StreamFrame<Value> =
		serde_json::from_str(&line).expect("stream response should decode");

	assert!(matches!(
		response.frame_type,
		monokit_daemon::StreamFrameType::LspOut
	));
...
",
	...
	for _ in 0..100 {
			...
			.expect("stream read should succeed")
		{
			...
			if matches!(
				response.frame_type,
				monokit_daemon::StreamFrameType::StreamError
			) {
				saw_eviction = true;
				assert_eq!(
					response.payload["code"],
					monokit_daemon::ERROR_SESSION_EVICTED_MEMORY
				);
				break;
			}
		}
