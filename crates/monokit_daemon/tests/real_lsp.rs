...
//!
//! ```sh
//! cargo test -p monokit_daemon --test real_lsp -- --ignored --test-threads=1
//! ```
...
fn unique_temp_dir(name: &str) -> PathBuf {
	let nanos = SystemTime::now()
		...
		.as_nanos();
	let dir = PathBuf::from("/tmp").join(format!("monokit-real-lsp-test-{name}-{nanos}"));
...
fn setup_fixture(name: &str) -> PathBuf {
	...
	// Copy monokit.toml
	fs::copy(src.join("monokit.toml"), dest.join("monokit.toml"))
		.expect("monokit.toml should copy");
