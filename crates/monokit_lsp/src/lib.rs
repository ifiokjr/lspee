...
impl LspRuntime {
	...
	pub async fn shutdown(&self) -> Result<()> {
		let shutdown_msg =
			json!({"jsonrpc":"2.0","id":"monokit-shutdown","method":"shutdown","params":null});
...
impl LspTransport {
	...
	pub fn prepare(&self, lsp: &monokit_config::LspConfig) -> Result<()> {
	...
	pub async fn spawn(&self, lsp: &monokit_config::LspConfig) -> Result<LspRuntime> {
		self.prepare(lsp)?;
...
/// ```
/// use serde_json::json;
/// use monokit_lsp::encode_lsp_frame;
///
...
mod tests {
	...
	fn unique_temp_dir(name: &str) -> PathBuf {
		...
		let dir = std::env::temp_dir().join(format!("monokit-lsp-{name}-{nanos}"));
