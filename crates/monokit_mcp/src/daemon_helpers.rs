//! Helpers that communicate with the monokit daemon over its Unix socket.
//!
//! The patterns here mirror `monokit_cli::commands::client` but are kept
...
use monokit_daemon::Stats;
use monokit_daemon::StatsOk;
...
use monokit_daemon::TYPE_STATS;
use monokit_daemon::TYPE_STATS_OK;
...
// ---------------------------------------------------------------------------
// Internal helpers (mirroring monokit_cli::commands::client)
// ---------------------------------------------------------------------------

fn daemon_socket_path(project_root: &Path) -> PathBuf {
	project_root.join(".monokit").join("daemon.sock")
}
...
fn spawn_daemon(project_root: &Path) -> Result<()> {
	...
	let log_dir = project_root.join(".monokit");
	...
	if let Ok(log_filter) = std::env::var("MONOKIT_LOG") {
		cmd.env("MONOKIT_LOG", log_filter);
	}

	if let Ok(log_format) = std::env::var("MONOKIT_LOG_FORMAT") {
		cmd.env("MONOKIT_LOG_FORMAT", log_format);
	}
...
fn build_attach_envelope(
	id: &str,
	resolved: &monokit_config::ResolvedConfig,
	lsp_id: &str,
	...
	let attach = Attach {
		...
		client_meta: ClientMeta {
			client_name: "monokit_mcp".to_string(),
...
///
/// Mirrors the logic in `monokit_cli::commands::capabilities`.
