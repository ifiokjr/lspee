```text
Terminal session
 └─ monokit call ...              CLI process (short-lived)
     └─ monokit serve             daemon process (long-lived, stdio=null)
...
When the daemon is eventually stopped (via `monokit stop` or system shutdown), it sends LSP `shutdown`/`exit` to each child, waits up to 2 seconds for graceful termination, then force-kills any remaining processes and removes the socket file.
